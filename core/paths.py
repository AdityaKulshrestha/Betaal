"""Centralized path resolution for Betaal.

Works both when running from source (``uv run betaal`` / ``python main.py``)
...
- User-writable data (models, config, database, speaker samples, OpenVINO
  compile cache) lives under ``~/.cache/betaal`` (honoring ``XDG_CACHE_HOME``).
...
def data_dir() -> Path:
    """User-writable base directory (``~/.cache/betaal``), created on demand."""
